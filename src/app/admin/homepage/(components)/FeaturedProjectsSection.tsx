...
export default function FeaturedProjectsSection({
	...
	const [projects, setProjects] = useState<IProjectPopulated[]>([]);
	...
	useEffect(() => {
		...
		Promise.all([
			...
			.catch(() => messageApi.error("Không thể tải danh sách công trình"))
	...
	const toggleSelect = (id: string, checked: boolean) => {
		if (checked && selected.length >= MAX_FEATURED) {
			messageApi.warning(`Chỉ được chọn tối đa ${MAX_FEATURED} công trình nổi bật`);
			return;
		}
		setSelected((prev) =>
	...
	const handleSave = () => {
		...
		adminFetch("/api/admin/homepage", {
			...
			.then((data) => {
				...
				messageApi.success("Đã lưu công trình nổi bật");
			})
			.catch(() => messageApi.error("Không thể lưu công trình nổi bật"))
		...
		<div className="flex flex-col h-full">
			...
			<div className="flex-1 min-h-0 overflow-auto">
				{loading ? (
					...
					<NoData description="Chưa có công trình nào">
						<Button onClick={() => router.replace("/admin/projects")}>
							Thêm công trình
						</Button>
					</NoData>
				) : (
					<div className="flex flex-col gap-2">
						{projects.map((project) => {
							...
							const isDisabled = !isChecked && selected.length >= MAX_FEATURED;
							return (
								<label
									key={id}
									className={`flex items-center gap-3 px-3 py-2 rounded-lg border transition-colors cursor-pointer ${isChecked ? "border-[#6366f1] bg-[#6366f1]/5" : "border-[#E8E6DE]"
								...
								>
									...
									<Text className="flex-1 truncate">{project.name}</Text>
								</label>
							);
						})}
					</div>
