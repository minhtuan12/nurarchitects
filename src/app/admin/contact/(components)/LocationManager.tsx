...
function createEmptyLocation(): LocationItem {
	return { name: "", address: "", lat: null, lng: null, link: "" };
}
...
export default function LocationManager({
	...
	const addressCollapses = value.map((location, index) => ({
		label: (
				...
				<Col span={24}>
					...
					<Space.Compact className="w-full">
						<Input
							...
							value={mapLinkDrafts[index] ?? location.link ?? ""}
							onChange={(e) => {
								...
								handleFieldChange(index, "link", url);
							}}
							disabled={disabled}
						/>
						<Button
